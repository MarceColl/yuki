...
Until later tasks fill them, create `codex.lisp`, `tool.lisp`, `context.lisp`, `agent.lisp`, `ui.lisp`, `app.lisp` each holding only `(in-package #:yuuki)`, and `t/codex.lisp`, `t/tool.lisp`, `t/agent.lisp`, `t/ui.lisp`, `t/app.lisp` each holding only `(in-package #:yuuki/test)`, so the system loads.
...
- Produces: `(instructions) → string`, `(user-item text)`, `(run-turn history prompt &key emit approve (stream #'stream-turn)) → history`. Events emitted: `(:call id code)`, `(:result id output)`, plus whatever `stream` emits. Test helpers `item-text`, `fake-stream`, `message-item`, `call-item` in `t/agent.lisp`, reused by Task 9's tests.
...
Append to `t/app.lisp` (`fake-stream` and `message-item` come from `t/agent.lisp`):
