...
(test boolean-accept-flow
  (let ((promise (sb-concurrency:make-mailbox))
        (spec '(:type :boolean :prompt "run?")))
    (multiple-value-bind (state effects)
        (reduce-all (yuuki::make-state :phase :running) (list (list :accept spec promise) '(:key #\y)))
      (is (eq :running (yuuki::state-phase state)))
      (is (equal (list (list :resolve promise t)) effects)))
    (multiple-value-bind (state effects)
        (reduce-all (yuuki::make-state :phase :running) (list (list :accept spec promise) '(:key #\n)))
      (is (eq :running (yuuki::state-phase state)))
      (is (equal (list (list :resolve promise nil)) effects)))
    (multiple-value-bind (state effects)
        (reduce-all (yuuki::make-state :phase :running) (list (list :accept spec promise) '(:key #\q)))
      (is (eq :accepting (yuuki::state-phase state)))
      (is (null effects)))))

(test choice-and-string-accept-flows
  (let ((promise (sb-concurrency:make-mailbox)))
    (multiple-value-bind (state effects)
        (reduce-all (yuuki::make-state :phase :running)
                    (list (list :accept '(:type :choice :prompt "which?" :options ("a" "b" "c" "d" "e" "f" "g" "h" "i" "j" "k" "l")) promise)
                          '(:key #\9) '(:key #\9) '(:key :enter) '(:key #\1) '(:key #\2) '(:key :enter)))
      (is (eq :running (yuuki::state-phase state)))
      (is (equal (list (list :resolve promise "l")) effects))
      (is (string= "" (chat-composer state)))
      (is (equal '((:user . "which?") (:plain . "  1. a") (:plain . "  2. b")) (subseq (chat-log state) 0 3))))
    (multiple-value-bind (state effects)
        (reduce-all (yuuki::make-state :phase :running)
                    (list (list :accept '(:type :string :prompt "name?") promise) '(:key #\j) '(:key #\o) '(:key :enter)))
      (is (equal (list (list :resolve promise "jo")) effects))
      (is (equal '((:user . "name?") (:user . "jo")) (chat-log state)))
      (is (string= "" (chat-composer state))))))

(test second-accept-while-accepting-is-rejected
  (let ((first (sb-concurrency:make-mailbox))
        (second (sb-concurrency:make-mailbox)))
    (multiple-value-bind (state effects)
        (reduce-all (yuuki::make-state :phase :running)
                    (list (list :accept '(:type :boolean) first) (list :accept '(:type :boolean) second)))
      (is (eq :accepting (yuuki::state-phase state)))
      (is (eq first (cdr (yuuki::state-accepting state))))
      (is (equal (list (list :resolve second nil)) effects)))))

(test show-hide-and-focus-cycle
  (let ((state (reduce-all (yuuki::make-state) '((:show "files" (("name" "bytes") ("a.lisp" 10))) (:key :tab) (:key :tab)))))
    (is (equal '("files") (mapcar #'yuuki::view-name (yuuki::state-views state))))
    (is (equal "files" (yuuki::state-focus state)))
    (let ((next (reduce-all state '((:key :page-down) (:key :page-down) (:key :page-up)))))
      (is (= 10 (yuuki::view-scroll (first (yuuki::state-views next))))))
    (let ((next (reduce-all state '((:key :tab)))))
      (is (eq :chat (yuuki::state-focus next))))
    (let ((next (reduce-all state '((:show "files" "replaced") (:hide "files")))))
...
(test render-paints-both-panes-and-status
  (let* ((state (reduce-all (yuuki::make-state :tail "partial")
                            '((:key #\h) (:key :tab) (:key #\z) (:repl-result "1" "=> 1")
                              (:show "files" (("name" "bytes") ("a.lisp" 10))))))
         (text (with-output-to-string (out) (yuuki::render out state 60 12))))
    (is (search "partial" text))
    (is (search "> h" text))
    (is (search "* z" text))
    (is (search "=> 1" text))
    (is (search "── files" text))
    (is (search "a.lisp" text))
    (is (search "tab: files" text))
  ...
  (let ((mailbox (sb-concurrency:make-mailbox))
    ...
    (unwind-protect
         (multiple-value-bind (stream seen)
           ...
           (let ((events (loop for (event ok) = (multiple-value-list (sb-concurrency:receive-message-no-hang mailbox))
                               while ok collect event)))
             (is (null (find :accept events :key #'first)))
    ...
    (unwind-protect
         (multiple-value-bind (stream seen)
           ...
           (let ((approve (loop for event = (sb-concurrency:receive-message mailbox :timeout 5)
                                until (or (null event) (eq (first event) :accept))
                                finally (return event))))
             (is (eq :boolean (getf (second approve) :type)))
...
(test view-selection-moves-and-calls-the-handler
  (let* ((seen (sb-concurrency:make-mailbox))
         (handler (lambda (row) (sb-concurrency:send-message seen row)))
         (state (reduce-all (yuuki::make-state)
                            (list (list :show "files" '(("name" "bytes") ("a" 1) ("b" 2) ("c" 3)) handler)
                                  '(:key :tab) '(:key :tab) '(:key :down) '(:key :down) '(:key :down) '(:key :up)))))
    (is (equal "files" (yuuki::state-focus state)))
    (is (= 1 (yuuki::view-cursor (first (yuuki::state-views state)))))
    (multiple-value-bind (next effects) (reduce-all state '((:key :enter)))
      (declare (ignore next))
      (is (equal (list (list :select handler '("b" 2))) effects))
      (yuuki::run-effect (first effects) (sb-concurrency:make-mailbox))
      (is (equal '("b" 2) (sb-concurrency:receive-message seen :timeout 2))))
    (let ((text (with-output-to-string (out) (yuuki::render out state 70 14))))
