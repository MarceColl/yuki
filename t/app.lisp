  ...
  (let ((promise (sb-concurrency:make-mailbox)))
    (multiple-value-bind (state effects)
        (reduce-all (yuuki::make-state :phase :running)
                    (list (list :accept '(:type :choice :prompt "which?" :options ("a" "b")) promise) '(:key #\9) '(:key #\2)))
      (is (eq :running (yuuki::state-phase state)))
      (is (equal (list (list :resolve promise "b")) effects))
      (is (equal '((:user . "which?") (:plain . "  1. a") (:plain . "  2. b")) (chat-log state))))
    (multiple-value-bind (state effects)
        (reduce-all (yuuki::make-state :phase :running)
