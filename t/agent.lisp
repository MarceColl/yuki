...
(test run-turn-invokes-lifecycle-hooks-and-adds-their-context
  (let ((yuuki::*hooks* (make-hash-table :test #'eq))
        (responses (list (cons (list (call-item "c1" "(+ 3 4)")) :stop)
                         (cons (list (message-item "done")) :stop)))
        (phases '())
        (instructions-seen '()))
    (flet ((hook (event)
             (let ((phase (getf event :event)))
               (push phase phases)
               (case phase
                 (:turn-start "turn context")
                 (:before-model (format nil "model step ~D" (getf event :step)))
                 (:after-model "model result context")
                 (:before-tool "tool input context")
                 (:after-tool
                  (is (search "=> 7" (getf event :output)))
                  "tool output context")
                 (:turn-end
                  (is (eq :complete (getf event :reason)))
                  nil)))))
      (dolist (phase '(:turn-start :before-model :after-model :before-tool :after-tool :turn-end))
        (yuuki::add-hook phase #'hook))
      (yuuki::run-turn
       '() "use a tool"
       :emit (lambda (event) (declare (ignore event)))
       :approve (lambda (id code) (declare (ignore id code)) t)
       :stream (lambda (instructions items emit &rest options)
                 (declare (ignore items emit options))
                 (push instructions instructions-seen)
                 (destructuring-bind (output . finish) (pop responses)
                   (values output finish))))
      (setf phases (nreverse phases)
            instructions-seen (nreverse instructions-seen))
      (is (equal '(:turn-start :before-model :after-model :before-tool
                   :after-tool :before-model :after-model :turn-end)
                 phases))
      (is (= 2 (length instructions-seen)))
      (is (search "turn context" (first instructions-seen)))
      (is (search "model step 1" (first instructions-seen)))
      (is (null (search "tool output context" (first instructions-seen))))
      (is (search "turn context" (second instructions-seen)))
      (is (search "model result context" (second instructions-seen)))
      (is (search "tool input context" (second instructions-seen)))
      (is (search "tool output context" (second instructions-seen)))
      (is (search "model step 2" (second instructions-seen))))))
